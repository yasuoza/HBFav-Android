...
public class EntryListFragment extends ListFragment implements PullToRefreshAttacher.OnRefreshListener {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        restoreActionBar();
        if (EntryListFeedManager.getList().isEmpty()) {
            EntryListFeedManager.replaceFeed(new FeedResponseHandler() {
                @Override
    ...
    public void onListItemClick(ListView listView, View view, int position, long id) {
        ...
        startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(EntryListFeedManager.get(position).getLink())));
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == EntryListFeedManager.getCategory()) {
            return false;
        }
        ...
        EntryListFeedManager.setCategory(item.getItemId());
        EntryListFeedManager.clearList();
        restoreActionBar();
        EntryListFeedManager.replaceFeed(new FeedResponseHandler() {
            @Override
            public void onSuccess() {
    ...
    private void restoreActionBar() {
        ...
        String title = getString(R.string.title_section2);
        if (mainActivity != null) {
            switch (EntryListFeedManager.getCategory()) {
    ...
    private class HotEntryListAdapter extends ArrayAdapter<Entry> {
        ...
        @Override
        public int getCount() {
            return EntryListFeedManager.getList().size();
        }

        @Override
        public Entry getItem(int position) {
            return EntryListFeedManager.getList().get(position);
        }

        @Override
        public View getView(int position, View view, ViewGroup parent) {
            final Entry entry = EntryListFeedManager.get(position);
